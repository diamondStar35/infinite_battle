...
class VirtualInput:
	def __init__(
		self, message="", password=False, whitelist=WHITELIST_ALL, value="", callback=None, hidden_message = "Hidden", repeat=True
	):
		self.text = value
				...
				if event.type == pygame.KEYDOWN:
					...
					if event.key == pygame.K_F2:
						if self.repeat==False:
							ngk.speak("Speak characters")
							self.repeat=True
						elif self.repeat==True:
							ngk.speak("Speak nothing")
							self.repeat=False
					if event.key == pygame.K_UP:
						...
						try:
							if event.unicode in self.allowed_characters and ngk.key_down(pygame.K_LCTRL)==False and ngk.key_down(pygame.K_RCTRL)==False and ngk.key_down(pygame.K_TAB)==False:
								if self.charindex < len(self.text):
...
def input_callback(input):
	if ngk.key_down(pygame.K_LCTRL) or ngk.key_down(pygame.K_RCTRL):
		if ngk.key_pressed(pygame.K_v):
			result=pyperclip.paste()
			input.text = input.text[:input.charindex] + result + input.text[input.charindex:]
			ngk.speak("Pasted")

def input_box(caption="", default_value=""):
